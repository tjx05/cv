...
class RootSIFTExtractor:
    """
    带有RootSIFT优化的特征提取器
    通过L1归一化和平方根映射，将SIFT特征的欧氏距离隐式转化为Hellinger距离，显著提升匹配精度。
    """
    def __init__(self,max_features=3000,use_rootsift=True):
        """
        初始化特征提取器
        输入：
            max_features: 每张图片最多提取的关键点数量（控制内存占用的关键参数）
            use_rootsift: 是否开启RootSIFT优化
        """
        # 声明底层特征提取器
    ...
    def extract(self,image_path,bbox=None):
        ...
        if self.use_rootsift:
            # L1归一化：将每个128维描述子向量除以其绝对值之和
            # 添加eps(1e-7)防止遇到全零向量导致除零报错
            eps=1e-7
