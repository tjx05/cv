...
def extract_and_save_all_features(image_dir, output_dir):
    """
    遍历整个图像库，提取 RootSIFT 特征并保存为 .npy 文件
    """
    # 确保输出目录存在
    ...
    if len(image_paths) == 0:
        print(f"错误：在 {image_dir} 中找不到任何 JPG 图像，请检查路径！")
        return

    print(f"找到 {len(image_paths)} 张图像，准备开始特征提取...")
    
    # 初始化特征提取器 (采用标准配置：最多 3000 个点，开启 RootSIFT)
