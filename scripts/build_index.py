...
def build_inverted_index(features_dir, vocab_path, index_path):
    ...
    # df (Document Frequency): 记录每个词出现在多少张不同的图片中
    ...
    for npy_file in tqdm(npy_files, desc="量化特征"):
        ...
        # 【核心算法】：用 KMeans 预测，把 128 维特征变成 1 维单词 ID
        words = kmeans.predict(descs)
        
        # 统计这张图片内每个单词出现的次数 (Term Frequency)
        word_counts = {}
        for w in words:
