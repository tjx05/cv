...
def geometric_verification(query_kps, query_descs, db_kps, db_descs):
    """
    RANSAC 核心逻辑：计算查询图和数据库图的内点数量
    """
    # 1. 暴力匹配描述子 (使用欧氏距离，开启交叉验证寻找最佳匹配)
    ...
    # 3. 使用 RANSAC 计算单应性矩阵
    # mask 矩阵中 1 代表符合几何变换的内点 (Inlier)，0 代表杂点 (Outlier)
...
def ransac_retrieval():
    ...
    print(f"\n🚀 开始 RANSAC 空间重排评测 (先取 Top-{TOP_N_PREFILTER} 初筛)...")
    for query_name, gt_info in tqdm(gt_data.items(), desc="RANSAC 检索中"):
        ...
        # 排序策略：优先按照内点数量降序，内点相同的按照原始倒排分数降序
    ...
    print("==================================================")
    print(f"🏆 RANSAC 系统构建完毕！最终 mAP 得分: {mAP_score:.4f}")
    print("==================================================")
